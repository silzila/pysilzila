        ...
        # for date fields, need to Parse as year, month, etc.. to aggreate
            ...
            if val['time_grain'] is not None:
                ...
                # which should be available in group by list but not in select list
                    ...
                    field_string_sort = f"EXTRACT(MONTH FROM {val['table_id']}.{val['field_name']})::INTEGER"
                ...
                # which should be available in group by list but not in select list
                    ...
                    field_string_sort = f"EXTRACT(DOW FROM {val['table_id']}.{val['field_name']})::INTEGER +1"
    ...
    for val in req["measures"]:
        ...
        # for date fields, parse to year, month, etc.. and then aggregate the field for Min & Max only
            ...
            # checking ('date')
                ...
                field_string = f"{val['aggr'].upper()}(DATE({val['table_id']}.{val['field_name']})) AS {val['field_name']}__{val['time_grain']}_{val['aggr']}"
            # checking ('dayofweek')
            ...
            elif val['aggr'] == 'countu' and val['time_grain'] in ('year', 'quarter', 'month', 'dayofmonth'):
                field_string = f"COUNT(DISTINCT(EXTRACT({period_dict[val['time_grain']]} FROM {val['table_id']}.{val['field_name']})::INTEGER)) AS {val['field_name']}__{val['time_grain']}_{val['aggr']}"
            # checking ('date')
            elif val['aggr'] == 'countu' and val['time_grain'] == 'date':
                field_string = f"COUNT(DISTINCT(DATE({val['table_id']}.{val['field_name']}))) AS {val['field_name']}__{val['time_grain']}_{val['aggr']}"
            # checking ('dayofweek')
            # In postgres, dayofweek starts at 0 not 1, so need to add 1 to the function
            elif val['aggr'] == 'countu' and val['time_grain'] == 'dayofweek':
                field_string = f"COUNT(DISTINCT(EXTRACT({period_dict[val['time_grain']]} FROM {val['table_id']}.{val['field_name']})::INTEGER) + 1) AS {val['field_name']}__{val['time_grain']}_{val['aggr']}"
