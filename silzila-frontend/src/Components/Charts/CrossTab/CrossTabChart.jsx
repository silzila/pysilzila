...
const CrossTabChart = ({ propKey, graphDimension, chartProp, updateCrossTabUserClicked }) => {
  let enable = false,
    ...
    dustbinColumns = [],
    dustbinValues = [],
    chartPropAxes = chartProp.properties[propKey].chartAxes,
    chartPropData = chartProp.properties[propKey].chartData;
  ...
  */
  if (chartPropAxes[0] && chartPropAxes[0].fields && chartPropAxes[0].fields.length > 0) {
    dustbinRows = CrossTab.cloneData(chartPropAxes[0].fields);
  }

  if (chartPropAxes[1] && chartPropAxes[1].fields && chartPropAxes[1].fields.length > 0) {
    dustbinColumns = CrossTab.cloneData(chartPropAxes[1].fields);
  }
    ...
    if (dustbinValues.length > 0) {
      ...
      for (let i = 0; i < previousColumnItems.length; i++) {
        dustbinValues.forEach((val) => {
          ...
          tempColumnObj.prefix = val.prefix;
