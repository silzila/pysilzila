...
revertAxes,})=>{
  ...
  let sliderRange = [0,0];
    ...
    const withPatternCollections = [{key : "begins_with" , value: "Start With"},
                                         {key : "ends_with", value: "Ends With"}, {key : "contains" , value : "Contains"},
                                         {key : "exact_match" , value : "Exact Match"}];
    const datePatternCollections = [{key: "year", value : "Year"}, {key : "quarter", value :  "Quarter"},
                                        {key: "month", value: "Month"}, {key:"yearquarter", value : "Year Quater"},
                                        {key:"yearmonth", value :"Year Month"}, {key: "date", value:"Date"},
                                        {key: "dayofmonth", value: "Day Of Month"},{key:"dayofweek", value :"Day Of Week"}];

    const datePatternSearchConditionCollections = [{key: "year", value : "Year"}, {key : "quarter", value :  "Quarter"},
  ...
  const setSliderRange = ()=>{
    if(["float","double","integer"].includes(dataType) && filterFieldData.exprType === "between"){
          if(filterFieldData.rawselectmembers && !filterFieldData.greaterThanOrEqualTo && !filterFieldData.lessThanOrEqualTo){
            filterFieldData.greaterThanOrEqualTo = filterFieldData.rawselectmembers[1];
            filterFieldData.lessThanOrEqualTo = filterFieldData.rawselectmembers[filterFieldData.rawselectmembers.length - 1];
          }             
  ...
  const handleSliderRangeOnChange = (event, newValue)=>{
    ...
    const DropDownForPattern = ({ items }) => {
      //let  _filterFieldData = _reverseResultForUI(filterFieldData);
        ...
        return (
            ...
            >
                {items.map((item) => {
                    return (
                        <option key={item.key} value={item.key} selected={item.key === filterFieldData.exprType}>
                            {item.value}
                        </option>
                    );
                })}
    ...
    const handleCustomRequiredValueOnBlur = (val, key) => {     
        ...
        if (key !== "exprInput") {
            if (filterFieldData.prefix === "date" && new Date(filterFieldData.greaterThanOrEqualTo) > new Date(filterFieldData.lessThanOrEqualTo)) {
                filterFieldData["isInValidData"] = true;
            } else {
                if (parseInt(filterFieldData.greaterThanOrEqualTo) > parseInt(filterFieldData.lessThanOrEqualTo)) {
                    filterFieldData["isInValidData"] = true;
                }
            }
        }
          ...
          <div ref={(node) => drag(drop(node))} className="UserFilterCard">
            ...
            {filterFieldData.dataType === "timestamp"|| filterFieldData.dataType == "date" ? (
              <div className="CustomRequiredField">
                 ...
                 :
                 <DropDownForDatePattern items={datePatternSearchConditionCollections}></DropDownForDatePattern>
    ...
    const mapDispatchToProps = (dispatch) => {
        return {
            updateLeftFilterItem: (propKey, bIndex, item) => dispatch(updateLeftFilterItem(propKey, bIndex, item)),
