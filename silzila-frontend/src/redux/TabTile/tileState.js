...
const tileStateReducer = (state = initialTileState, action) => {
    switch (action.type) {
        case "ADD_TILE":
        ...
        case "REMOVE_TILES_OF_TAB":
            ...
            return update(state, { tiles: { $unset: tilesToRemove }, tileList: { $unset: [action.payload.tabId] } });

        case "RENAME_TILE":
            let tileKey2 = `${action.payload.tabId}.${action.payload.tileId}`;
            return update(state, { tiles: { [tileKey2]: { tileName: { $set: action.payload.renameValue } } } });
        ...
        case "TOGGLE_GRAPH_SIZE":
            return update(state, { tiles: { [action.payload.tileKey]: { graphSizeFull: { $set: action.payload.graphSize } } } });
