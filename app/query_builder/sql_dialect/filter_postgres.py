    ...
    # get DATE dictinct values
        ...
        if req['filter_type'] == 'select members':
            ...
            if req['aggr'] == 'year':
                ...
                field1 = f"EXTRACT(QUARTER FROM {req['table_id']}.{req['field_name']})::INTEGER AS index"
                field2 = f"CONCAT('Q', EXTRACT(QUARTER FROM {req['table_id']}.{req['field_name']})::INTEGER) AS colmn"
                QUERY = f"SELECT DISTINCT\n\t{field1}, \n\t{field2}\nFROM\n\t{FROM_TBL}\n\nORDER BY\n\t1"
